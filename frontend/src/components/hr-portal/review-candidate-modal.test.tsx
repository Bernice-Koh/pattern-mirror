...
describe('ReviewCandidateModal', () => {
  ...
  it('rejects through the addition-scoped decision', async () => {
    getProposalAuditMock.mockResolvedValue(AUDIT)
    decideAdditionMock.mockResolvedValue(undefined)
    ...
    await waitFor(() =>
      expect(decideAdditionMock).toHaveBeenCalledWith('a1', 'reject'),
    )
  })
  ...
  it('cancels without deciding', () => {
    ...
    expect(decideAdditionMock).not.toHaveBeenCalled()
  })

  it('degrades when the audit and citation are unavailable', async () => {
    getProposalAuditMock.mockRejectedValue(new Error('boom'))
    ...
    expect(
      await screen.findByText('The agent reasoning is unavailable.'),
    ).toBeInTheDocument()
    expect(
      screen.getByText('No citation is attached to this phrase.'),
    ).toBeInTheDocument()
  })

  it('surfaces a decision conflict and keeps the modal open', async () => {
    getProposalAuditMock.mockResolvedValue(AUDIT)
    const conflict = new GrowthError(409)
    decideAdditionMock.mockRejectedValue(conflict)
    const onClose = vi.fn()
